import pandas as pd
import numpy as np
import scipy.stats as spice
from scipy.stats import norm
from scipy.optimize import minimize
def drawdown(return_series: pd.Series):
    """
    Takes a time series of returns, computes and returns a dataframe that contains:
    wealth index
    peaks
    draw
    """
...
def get_hfi_returns():
    ...
    """
    import pandas as pd
    hfi=pd.read_csv('Downloads/data/edhec-hedgefundindices.csv', header=0, index_col=0, na_values=-99.99, parse_dates= True)
    hfi=hfi/100
...
def get_ind_returns():
    """
    Load and Format the french 30 industry posrtfolio value weighted returns
    """
    
    ind = pd.read_csv("Downloads/data/ind30_m_vw_rets.csv",header=0,index_col=0,parse_dates=True)/100
...
def get_ind_size():
    """
    Load and Format the french 30 industry posrtfolio sizes
    """
    
    ind = pd.read_csv("Downloads/data/ind30_m_size.csv",header=0,index_col=0,parse_dates=True)
    ind.index=pd.to_datetime(ind.index,format="%Y%m").to_period('M')
    ind.columns = ind.columns.str.strip()
    return ind
...
def get_ind_nfirms():
    ...
    ind = pd.read_csv("Downloads/data/ind30_m_nfirms.csv",header=0,index_col=0,parse_dates=True)
    ind.index=pd.to_datetime(ind.index,format="%Y%m").to_period('M')
    ind.columns = ind.columns.str.strip()
    return ind
def semideviation(r):
    """
    Returns the semideviation aka negative semid
    """
    is_negative = r<0
...
def is_normal(r,level=0.01):
    statistic,p_value= spice.jarque_bera(r)
    return p_value>0.01

def var_historic(r,level=5):
    """
    var historic, r is pandas dataframe, or series
    """
...
def var_gaussian(r,level=5,modified=False):
    """
    Returns the parametric gaussian Vara
    """
...
def fixedmix_allocator(r1,r2, w1, **kwargs):
    """
    produces time series with PSP weights
    """
    return pd.DataFrame(data=w1, index = r1.index , columns = r1.columns)

def bt_mix(r1,r2, allocator, **kwargs):
    """
    back test of allocating r1 and r2
    """
    if not r1.shape== r2.shape:
        raise ValueError("r1 and r2 need to be the same shape")
    weights = allocator(r1,r2,**kwargs)
    if not weights.shape == r1.shape:
        raise ValueError("weights dont match shape")
    r_mix = weights*r1 + (1-weights)*r2
    return r_mix
        
def terminal_values(rets):
    """
    returns final values of a dollar at the end of the return period
    """
    return (rets+1).prod()
...
def get_fff_returns():
    ...
    """
    rets = pd.read_csv("Downloads/MOOC/data/F-F_Research_Data_Factors_m.csv",
